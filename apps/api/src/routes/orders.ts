...
router.post('/:id/shipments', can('OPERATIONS_MANAGE'), async (req, res, next) => {
  try {
    ...
    const schema = z.object({
      ...
      // Additional cost fields matching quotation cost types
      packagingCharges: z.number().nonnegative().optional(),
      insuranceCharges: z.number().nonnegative().optional(),
      inspectionCharges: z.number().nonnegative().optional(),
      commissionCharges: z.number().nonnegative().optional(),
...
router.put('/:orderId/shipments/:shipmentId', can('OPERATIONS_MANAGE'), async (req, res, next) => {
  try {
    ...
    const schema = z.object({
      ...
      // Additional cost fields matching quotation cost types
      packagingCharges: z.number().nonnegative().nullable().optional(),
      insuranceCharges: z.number().nonnegative().nullable().optional(),
      inspectionCharges: z.number().nonnegative().nullable().optional(),
      commissionCharges: z.number().nonnegative().nullable().optional(),
